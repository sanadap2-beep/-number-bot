...
class GenericJsonProtocol(BaseProtocol):
                ...
                async with session.request(
                    ...
                    **request_kwargs,
                    ...
                    if response.status in (401, 403):
                        raise ProtocolError(
                            f"فشل مصادقة المزود: HTTP {response.status}"
                        )
                    ...
                    try:
                        ...
                        raise ProtocolError(
                            f"استجابة JSON غير صالحة: {body[:200]}"
                        ) from None
    ...
    async def get_balance(self) -> ProtocolBalance:
        data = await self._request(
            "balance", self._method("balance", "GET")
        )
        balance = _deep_get(
            data,
    ...
    async def get_services(self) -> list[ProtocolService]:
        data = await self._request(
            "services", self._method("services", "GET")
        )
        ...
        for item in services_data:
            ...
            result.append(
                ProtocolService(
                    ...
                    requires_player_id=bool(_deep_get(item, field_map["requires_player_id"], False)),
    ...
    async def place_order(
        ...
        data = await self._request(
            "order", self._method("order", "POST"), payload
        )
