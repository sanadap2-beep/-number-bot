...
async def overview(session=Depends(get_session)):
    ...
    counts = {
        key: (await session.execute(query)).scalar_one()
...
async def providers(session=Depends(get_session)):
    providers = list((await session.execute(select(ApiProvider).order_by(ApiProvider.priority, ApiProvider.id))).scalars().all())
    statuses = {
    ...
    return [
        {
            ...
            "sms_status": statuses.get(provider.name, {}).is_online if provider.name in statuses else None,
        }
