    ...
    async def report(session, days: int = 30) -> dict:
        ...
        numbers = (
            await session.execute(
                select(NumberOrder).where(NumberOrder.purchased_at >= since)
            )
        ).scalars().all()
        api_orders = (
            await session.execute(
                select(UnifiedOrder).where(UnifiedOrder.created_at >= since)
            )
        ).scalars().all()
        ...
        result["sms"] = sorted(providers.values(), key=lambda row: row["success_rate"], reverse=True)
