...
class InventoryService:
    ...
    def _cipher() -> Fernet:
        ...
        if not key:
            raise InventoryError(
                "ضع INVENTORY_ENCRYPTION_KEY قبل استخدام مخزون المنتجات."
            )
        try:
            return Fernet(key.encode())
        except (ValueError, TypeError):
            raise InventoryError("مفتاح تشفير المخزون غير صالح.") from None
    ...
    def decrypt_value(encrypted_value: str) -> str:
        try:
            return InventoryService._cipher().decrypt(
                encrypted_value.encode()
            ).decode()
        except (InvalidToken, UnicodeDecodeError):
            raise InventoryError(
                "تعذر فك تشفير عنصر المخزون؛ تحقق من المفتاح."
            ) from None
    ...
    async def add_item(
        ...
        if product.fulfillment_type != ProductFulfillmentType.INVENTORY:
            raise InventoryError(
                "المنتج غير مضبوط كمنتج مخزون رقمي."
            )
        value = value.strip()
        if not value or len(value) > 1500:
            raise InventoryError("قيمة المخزون فارغة أو طويلة جداً.")

        encrypted = InventoryService._cipher().encrypt(value.encode()).decode()
