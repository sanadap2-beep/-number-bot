...
class PromotionService:
    ...
    def calculate_discount(
        ...
        return min(amount, max(Decimal("0"), discount)).quantize(
            Decimal("0.0001")
        )
    ...
    async def get_best_promotion(
        ...
        promotions = await PromotionService.get_active_for_product(
            session, product_id
        )
        if not promotions:
        ...
        best = max(
            promotions,
            key=lambda promotion: PromotionService.calculate_discount(
                promotion, amount
            ),
        )
