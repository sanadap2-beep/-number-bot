...
class LoyaltyService:
        ...
        async with BalanceService._get_lock(user_id):
            ...
            existing = await session.execute(
                select(LoyaltyEvent).where(
                    LoyaltyEvent.event_key == event_key
                )
            )
            ...
            user.loyalty_points = max(
                0, (user.loyalty_points or 0) + points
            )
            session.add(
                LoyaltyEvent(
                    user_id=user_id,
    ...
    async def award_purchase_points(
        ...
        multiplier = LoyaltyService.tier_for_points(
            current_points or 0
        ).points_multiplier
        points = int(
            (
                amount_usd * rate * multiplier
    ...
    async def claim_daily(
        ...
        today = date.today()
        base_points = await SettingsService.get_int(
            "loyalty_daily_points", 25
        )
        async with BalanceService._get_lock(user_id):
            user = await session.get(User, user_id)
            if user is None:
    ...
    async def redeem_points(
        ...
        ratio = await SettingsService.get_int(
        ...
        if ratio <= 0 or points < minimum:
            raise LoyaltyError(
                f"الحد الأدنى للاستبدال هو {minimum} نقطة."
            )
        amount = (Decimal(points) / Decimal(ratio)).quantize(
