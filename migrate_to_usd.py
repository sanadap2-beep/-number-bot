...
def rub_to_usd(amount_rub: Decimal, rate: Decimal) -> Decimal:
    ...
    return (amount_rub / rate).quantize(
...
async def _tables(connection: AsyncConnection) -> set[str]:
    if connection.dialect.name == "sqlite":
        ...
        raise RuntimeError(
            "هذا السكربت يدعم SQLite فقط؛ استخدم migration مناسبة لقاعدة أخرى."
        )
...
async def _convert_column(
    ...
    for row in rows:
        ...
        assignments = ", ".join(
            f"{column} = :value_{index}"
            for index, column in enumerate(amount_columns)
        )
        params = {
            f"value_{index}": str(value)
    ...
    async with engine.begin() as connection:
        ...
        if "balance" in users_columns:
            count = await _convert_column(
                connection, "users", "id", ("balance",), rate
            )
        ...
        if "deposit_requests" in tables:
            ...
            if "amount_rub" in deposit_columns:
                ...
                for deposit_id, amount_rub in rows:
                    amount_usd = rub_to_usd(
                        _decimal(amount_rub), rate
                    )
                    await connection.execute(
                        text(
            ...
            for legacy, current in legacy_pairs:
                if legacy in order_columns:
                    ...
                    for order_id, legacy_value in result.fetchall():
                        current_value = rub_to_usd(
                            _decimal(legacy_value), rate
                        )
                        await connection.execute(
                            text(
        ...
        if "transfers" in tables:
            ...
            if "amount" in transfer_columns:
                count = await _convert_column(
                    connection, "transfers", "id", ("amount",), rate
                )
        ...
        for table, name, definition in (
            ...
            if table in tables:
                ...
                await _add_column(
                    connection, table, columns, name, definition
                )

        if "settings" in tables:
            await connection.execute(
                text(
                    "DELETE FROM settings WHERE key = 'exchange_rate_usd_rub'"
                )
            )
            defaults = {
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="ترحيل قاعدة بيانات SQLite القديمة من RUB إلى USD"
    )
    parser.add_argument(
        "--from-rub",
        action="store_true",
