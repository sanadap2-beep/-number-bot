...
async def my_product_requests(
    ...
    requests = await ProductRequestService.get_for_user(
        session, db_user.id, limit=20
    )
    await callback.answer()
    if not requests:
        await callback.message.edit_text(
...
async def product_request_vote(
    ...
    request_id = int(callback.data.split(":")[3])
    try:
        voted = await ProductRequestService.vote(
            session, request_id, db_user.id
        )
    except ProductRequestError as exc:
    ...
    await product_request_view(
        callback, session, db_user, answer_callback=False
    )
