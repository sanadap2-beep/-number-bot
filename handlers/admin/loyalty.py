...
async def loyalty_dashboard(callback: CallbackQuery, session):
    ...
    participants = (
        await session.execute(
            select(func.count(User.id)).where(User.loyalty_points > 0)
        )
    ).scalar_one()
    ...
    daily_points = await SettingsService.get_int(
        "loyalty_daily_points", 25
    )
    tiers = "\n".join(
        f"{tier.emoji} {tier.name}: {tier.minimum_points} نقطة "
        f"(x{tier.points_multiplier})"
        for tier in TIERS
    )
    top_text = "\n".join(
        f"{index}. {user.full_name or user.telegram_id}: "
        f"{user.loyalty_points} نقطة"
