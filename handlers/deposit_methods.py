...
async def usdt_auto_amount_received(
    ...
    max_deposit = await SettingsService.get_decimal(
    ...
    if amount > max_deposit:
        await message.answer(
            f"⚠️ الحد الأقصى للشحن هو {max_deposit}$"
        )
        return
...
async def usdt_auto_check_button(
    callback: CallbackQuery, session, bot, db_user: User
):
    invoice_id = int(callback.data.split(":")[1])
    invoice = await _get_owned_invoice(
        session, invoice_id, db_user.id
