...
async def _render_loyalty(target, session, db_user: User):
    ...
    if next_tier:
        progress_text = (
            f"📈 التقدم نحو {next_tier.emoji} {next_tier.name}:\n"
            f"{_progress_bar(summary['progress'], summary['required'])}\n"
            f"{summary['progress']} / {summary['required']} نقطة"
        )
    else:
        progress_text = "👑 وصلت إلى أعلى مستوى!"

    text = (
        "🎁 <b>برنامج الولاء والمكافآت</b>\n\n"
        f"مستواك الحالي: {tier.emoji} <b>{tier.name}</b>\n"
        f"💎 نقاطك: <b>{points}</b>\n"
        f"🔥 سلسلة التسجيل اليومي: <b>{summary['streak']} يوم</b>\n"
        f"✨ الميزة: {tier.benefit}\n"
        f"⚡ معامل النقاط: x{tier.points_multiplier}\n\n"
        f"{progress_text}\n\n"
        "اجمع النقاط من المشتريات والتسجيل اليومي، "
        "ثم استبدلها برصيد قابل للاستخدام."
    )
...
async def loyalty_daily(
    ...
    points, streak, already_claimed = await LoyaltyService.claim_daily(
        session, db_user.id
    )
    if already_claimed:
        await callback.answer(
...
async def loyalty_redeem(
    ...
    try:
        amount = await LoyaltyService.redeem_points(
            session, db_user.id, points
        )
